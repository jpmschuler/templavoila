...
class NewContentElementController extends \TYPO3\CMS\Backend\Module\BaseScriptClass
{
    ...
    public function mainAction(\Psr\Http\Message\ServerRequestInterface $request, \Psr\Http\Message\ResponseInterface $response)
    {
       $this->postVars = $request->getParsedBody();
        $this->init();
        $this->main();
        $response->getBody()->write($this->doc->insertStylesAndJS($this->content));
        return ($response);
    }
    ...
    public function init()
    {
        ...
        $this->doc->setModuleTemplate('EXT:templavoila/Resources/Private/Templates/mod1_new_content_7.6.html');
        // $this->doc->getPageRenderer()->loadPrototype();
        
        //$this->doc->getPageRenderer()->addCssFile($GLOBALS['BACK_PATH'] . \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('templavoila') . 'Resources/Public/StyleSheet/mod1_7.6.css');
        $this->doc->JScodeLibArray['prototype'] = '<script src="' . \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('templavoila') . 'Resources/Public/JavaScript/contrib/prototype/prototype.js" type="text/javascript"></script>';
        
    //    $this->doc->getPageRenderer()->addHeaderData('<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js" integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa" crossorigin="anonymous"></script>');
    ...
    public function main()
    {
        if ($this->id && $this->access) {
            ...
            // Wrapper for wizards
            $this->elementWrapper['sectionHeader'] = array(
                '<h3 class="bgColor5">',
                '</h3>'
            );
            ...
            $this->elementWrapper['wizard'] = array(
                '<div class="media">',
                '</div>'
            );
            $this->elementWrapper['wizardPart'] = array(
                '',
                ''
            );
           ...
           /* $this->doc->JScode = $this->doc->wrapScriptTags('
            ...
            if ($this->config['renderMode'] == 'tabs') {
                $code = \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('sel1', TRUE) . '<br /><br />' . $this->doc->getDynamicTabMenu($menuItems, 'new-content-element-wizard');
            } else {
    ...
    protected function getDocHeaderButtons()
    {
        ...
        if ($this->returnUrl) {
            ...
            $buttons['back'] = '<a href="' . htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::linkThisUrl($this->returnUrl)) . '" class="typo3-goBack" title="' . \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:lang/locallang_core.xlf:labels.goBack', TRUE) . '">' . $backIcon . '</a>';
        }
    ...
    {
        return array(
            'header' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL($wizardGroup['header'])
        );
    }
     ...
     *
     * @param
     *            array &$wizardItems Wizard items, passed by reference
     *            
     * @return void
     */
    public function removeInvalidElements(&$wizardItems)
    {
        ...
        $TCEFORM_TSconfig = \TYPO3\CMS\Backend\Utility\BackendUtility::getTCEFORM_TSconfig('tt_content', array(
            'pid' => $this->id
        ));
     ...
     *
     * @param string $table
     *            Table nme to fetch records from
     *            
     ...
     *
     * @param \Extension\Templavoila\Domain\Model\Template $toObj
     *            LocalProcessing as array
     *            
