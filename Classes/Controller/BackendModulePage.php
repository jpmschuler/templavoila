...
class BackendModulePage extends \TYPO3\CMS\Backend\Module\BaseScriptClass {
     ...
     * (with version, sheet and language currently set).
     * Mainly used for showing "unused elements" in sidebar.
     *
     * @var array
     */
     ...
     * Used for displaying the flags for content elements, set in init().
     *
     * @var array
     */
    ...
    /**
     * Permissions for the parrent record (normally page).
     * Used for hiding icons.
     *
     * @var integer
     */
    public $calcPerms;
    ...
    public function main()
    {
        ...
        if ($access) {
                    ...
                    if ($result !== FALSE) {
                        ...
                        if (\Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->isPSet($this->calcPerms, 'pages', 'edit')) {
                            ...
                            $iconEdit = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-open', array(
                                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:lang/locallang_mod_web_list.xlf:editPage')
                            ));
            ...
            if ($render_editPageScreen) {
                ...
                if ($this->rootElementRecord['content_from_pid']) {
                    ...
                    $link = '<a href="' . $this->link_createCSRFurl([
                        'id' => (int) $this->rootElementRecord['content_from_pid']
                    ]) . '">' . htmlspecialchars($title) . ' (PID ' . (int) $this->rootElementRecord['content_from_pid'] . ')</a>';
                    /** @var \TYPO3\CMS\Core\Messaging\FlashMessage $flashMessage */
                    $flashMessage = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Messaging\FlashMessage::class, '', sprintf(\Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('content_from_pid_title'), $link), \TYPO3\CMS\Core\Messaging\FlashMessage::INFO);
                    $editCurrentPageHTML = '';
            ...
            if ($cmd == 'crPage') { // create a new page
                ...
                if (! isset($pageInfoArr['uid'])) {
                    $flashMessage = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Messaging\FlashMessage::class, \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('page_not_found'), \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('title'), \TYPO3\CMS\Core\Messaging\FlashMessage::INFO);
                    $this->content .= $flashMessage->render();
                } else {
                    $flashMessage = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Messaging\FlashMessage::class, \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('default_introduction'), \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('title'), \TYPO3\CMS\Core\Messaging\FlashMessage::INFO);
                    $this->content .= $flashMessage->render();
                }
            }
        }
        ...
        $content .= $this->doc->moduleBody(array(), $this->getDocHeaderButtons(! isset($pageInfoArr['uid'])), $this->getBodyMarkers());
     ...
     *
     * @param boolean $noButtons
     *            Determine whether to show any icons or not
     *
    ...
    protected function getDocHeaderButtons($noButtons = FALSE)
    {
        ...
        $buttons['view'] = '<a href="#" class="btn btn-default btn-sm " onclick="' . htmlspecialchars(\TYPO3\CMS\Backend\Utility\BackendUtility::viewOnClick($this->id, $BACK_PATH, \TYPO3\CMS\Backend\Utility\BackendUtility::BEgetRootLine($this->id), '', '', $viewAddGetVars)) . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-view', array(
            'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:lang/locallang_core.php:labels.showPage', 1)
        )) . '</a>';
        ...
        if (\Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->check('modules', 'web_list')) {
            ...
            $buttons['record_list'] = '<a href="' . htmlspecialchars($href) . '" class="btn btn-default btn-sm ">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-system-list-open', array(
                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:lang/locallang_core.php:labels.showList', 1)
            )) . '</a>';
        }
        ...
        if (! $this->modTSconfig['properties']['disableIconToolbar']) {
            ...
            // Page history
            $buttons['history_page'] = '<a href="#" class="btn btn-default btn-sm " onclick="' . htmlspecialchars('jumpToUrl(\'' . $BACK_PATH . 'show_rechis.php?element=' . rawurlencode('pages:' . $this->id) . '&returnUrl=' . rawurlencode(\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REQUEST_URI')) . '#latest\');return false;') . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-history-open', array(
                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:cms/layout/locallang.xlf:recordHistory', 1)
            )) . '</a>';
            
            if (! $this->translatorMode && \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->isPSet($this->calcPerms, 'pages', 'new')) {
                // Create new page (wizard)
                $buttons['new_page'] = '<a href="#" class="btn btn-default btn-sm " onclick="' . htmlspecialchars('jumpToUrl(\'' . $BACK_PATH . 'db_new.php?id=' . $this->id . '&pagesOnly=1&returnUrl=' . rawurlencode(\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REQUEST_URI') . '&updatePageTree=true') . '\');return false;') . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-page-new', array(
                    'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:cms/layout/locallang.xlf:newPage', 1)
                )) . '</a>';
            }
            ...
            if (! $this->translatorMode && \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->isPSet($this->calcPerms, 'pages', 'edit')) {
                ...
                $buttons['edit_page'] = '<a href="#" class="btn btn-default btn-sm " onclick="' . htmlspecialchars(\TYPO3\CMS\Backend\Utility\BackendUtility::editOnClick($params, $BACK_PATH)) . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-open', array(
                    'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:cms/layout/locallang.xlf:editPageProperties', 1)
                )) . '</a>';
                // Move page
                $buttons['move_page'] = '<a href="' . htmlspecialchars($BACK_PATH . 'move_el.php?table=pages&uid=' . $this->id . '&returnUrl=' . rawurlencode(\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REQUEST_URI'))) . '" class="btn btn-default btn-sm ">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-page-move', array(
                    'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:cms/layout/locallang.xlf:move_page', 1)
                )) . '</a>';
            }
            ...
            if ($this->id) {
                $cacheUrl = $GLOBALS['BACK_PATH'] . 'tce_db.php?vC=' . \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->veriCode() . \TYPO3\CMS\Backend\Utility\BackendUtility::getUrlToken('tceAction') . '&redirect=' . rawurlencode(\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REQUEST_URI')) . '&cacheCmd=' . $this->id;
    ...
    public function render_editPageScreen()
    {
        ...
        if (is_array($this->altRoot)) {
            $output .= '<div style="text-align:right; width:100%; margin-bottom:5px;"><a href="' . $this->link_createCSRFurl([
                'altRoot' => null
            ]) . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-view-go-back', array(
                'title' => htmlspecialchars(\Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('goback'))
            )) . '</a></div>';
        }
        ...
        if (! \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->isAdmin() && $this->modTSconfig['properties']['enableContentAccessWarning']) {
            if (! ($this->hasBasicEditRights())) {
                ...
                $message = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Messaging\FlashMessage::class, \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('missing_edit_right_detail'), \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('missing_edit_right'), \TYPO3\CMS\Core\Messaging\FlashMessage::INFO);
        ...
        if ((\Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->isAdmin() || $this->modTSconfig['properties']['enableOutlineForNonAdmin']) && $this->MOD_SETTINGS['showOutline']) {
     ...
     *            DataStructure info array (the whole tree)
     * @param string $languageKey
     *            Language key for the display
     * @param array $parentPointer
     *            Flexform Pointer to parent element
     * @param array $parentDsMeta
     *            Meta array from parent DS (passing information about parent containers localization mode)
     *
     * @return string HTML
     * @access protected
    ...
    /**
     * Renders the display framework of a single sheet.
     ...
     *            Language key for the display
     * @param string $sheet
     *            The sheet key of the sheet which should be rendered
     * @param array $parentPointer
     *            Flexform pointer to parent element
     * @param array $parentDsMeta
     *            Meta array from parent DS (passing information about parent containers localization mode)
    ...
    public function render_framework_singleSheet($contentTreeArr, $languageKey, $sheet, $parentPointer = array(), $parentDsMeta = array())
    {
        ...
        switch ($contentTreeArr['el']['table']) {
            ...
            case 'tt_content':
                ...
                if (! $this->translatorMode) {
                    ...
                    if ($canEditContent) {
                        $iconMakeLocal = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('extensions-templavoila-makelocalcopy', array(
                            'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('makeLocal')
                        ));
                        ...
                        if ($this->modTSconfig['properties']['enableDeleteIconForLocalElements'] < 2 || ! $elementBelongsToCurrentPage || $this->global_tt_content_elementRegister[$contentTreeArr['el']['uid']] > 1) {
                            $iconUnlink = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('extensions-templavoila-unlink', array(
                                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('unlinkRecord')
                            ));
                    ...
                    if ($canEditElement && \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->recordEditAccessInternals('tt_content', $contentTreeArr['previewData']['fullRow'])) {
                        if (($elementBelongsToCurrentPage || $this->modTSconfig['properties']['enableEditIconForRefElements']) && ! in_array('edit', $this->blindIcons)) {
                            $iconEdit = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-open', array(
                                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('editrecord')
                            ));
                        ...
                        if ($canEditContent && $this->modTSconfig['properties']['enableDeleteIconForLocalElements'] && $elementBelongsToCurrentPage) {
                            ...
                            $iconDelete = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-edit-delete', array(
                                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('deleteRecord')
                            ));
        ...
        if ($this->allAvailableLanguages[$languageUid]['flagIcon']) {
            $languageIcon = \Extension\Templavoila\Utility\IconUtility::getFlagIconForLanguage($this->allAvailableLanguages[$languageUid]['flagIcon'], array(
                'title' => $languageLabel,
                'alt' => $languageLabel
            ));
        } else {
        ...
        if ($contentTreeArr['el']['table'] == 'pages') {
            $finalContent = '
			<div>
		    <h1>' . ($elementBelongsToCurrentPage ? '' : '<em>') . htmlspecialchars($title) . ($elementBelongsToCurrentPage ? '' : '</em>') . $titleBarLeftButtons . $languageIcon . '</h1>' . ($warnings ? '<div class="tpm-warnings">' . $warnings . '</div>' : '') . $this->render_framework_subElements($contentTreeArr, $languageKey, $sheet, $calcPerms) . '<div class="tpm-preview exampleContent">' . $previewContent . '</div>' . $this->render_localizationInfoTable($contentTreeArr, $parentPointer, $parentDsMeta) . '
		</div>
	';
        } else {
            $finalContent = '<div class="' . $elementClass . '">' . '<a name="c' . md5($this->apiObj->flexform_getStringFromPointer($this->currentElementParentPointer) . $contentTreeArr['el']['uid']) . '"></a>' . '<div class="tpm-titlebar t3-page-ce-header ' . $elementTitlebarClass . '">' . '<div class="t3-page-ce-header-icons-left">' . $titleBarLeftButtons . $languageIcon . '</div>' . '<div class="t3-page-ce-header-icons-right">' . $titleBarRightButtons . '</div>' . '</div>' . '<div class="t3-page-ce-body"><div class="t3-page-ce-body-inner">' . '<strong>' . ($elementBelongsToCurrentPage ? '' : '<em>') . htmlspecialchars($title) . ($elementBelongsToCurrentPage ? '' : '</em>') . '</strong><br/>' . ($warnings ? '<div class="tpm-warnings">' . $warnings . '</div>' : '') . $this->render_framework_subElements($contentTreeArr, $languageKey, $sheet, $calcPerms) . '<div class="tpm-preview exampleContent">' . $previewContent . '</div>' . $this->render_localizationInfoTable($contentTreeArr, $parentPointer, $parentDsMeta) . '</div>' . '</div>' . '</div>';
        }
    ...
    /**
     * Renders the sub elements of the given elementContentTree array.
     ...
     *            Content tree starting with the element which possibly has sub elements
     * @param string $languageKey
     *            Language key for current display
     * @param string $sheet
     *            Key of the sheet we want to render
     * @param integer $calcPerms
     *            Defined the access rights for the enclosing parent
     *
     * @throws RuntimeException
     *
        ...
        if ($elementContentTreeArr['el']['table'] == 'pages' && $langDisable != 1 && $langChildren == 1) {
            if ($this->disablePageStructureInheritance($elementContentTreeArr, $sheet, $lKey, $vKey)) {
                ...
                if (! \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->isAdmin()) {
                    /** @var \TYPO3\CMS\Core\Messaging\FlashMessage $flashMessage */
                    $flashMessage = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Messaging\FlashMessage::class, \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('page_structure_inherited_detail'), \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('page_structure_inherited'), \TYPO3\CMS\Core\Messaging\FlashMessage::INFO);
                    $this->flashMessageService->getMessageQueueByIdentifier('ext.templavoila')->enqueue($flashMessage);
                }
            }
        }
        ...
        foreach ($elementContentTreeArr['sub'][$sheet][$lKey] as $fieldID => $fieldValuesContent) {
            ...
            if (is_array($fieldValuesContent[$vKey]) && ($elementContentTreeArr['previewData']['sheets'][$sheet][$fieldID]['isMapped'] || $elementContentTreeArr['previewData']['sheets'][$sheet][$fieldID]['type'] == 'no_map') && $elementContentTreeArr['previewData']['sheets'][$sheet][$fieldID]['tx_templavoila']['preview'] != 'disable') {
                ...
                if (isset($elementContentTreeArr['previewData']['sheets'][$sheet][$fieldID]['TCEforms']['config']['maxitems'])) {
                    ...
                    if ($maxItemsReached) {
                        /** @var \TYPO3\CMS\Core\Messaging\FlashMessage $flashMessage */
                        $flashMessage = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Messaging\FlashMessage::class, '', sprintf(\Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('maximal_content_elements'), $maxCnt, $elementContentTreeArr['previewData']['sheets'][$sheet][$fieldID]['tx_templavoila']['title']), \TYPO3\CMS\Core\Messaging\FlashMessage::INFO);
                        $this->flashMessageService->getMessageQueueByIdentifier('ext.templavoila')->enqueue($flashMessage);
                    }
                }
            ...
            if ($adminOnly == 'strict') {
                ...
                if ($adminOnly == 'fallback' && isset($elementContentTreeArr['sub'][$sheet][$lKey])) {
                    foreach ($elementContentTreeArr['previewData']['sheets'][$sheet] as $fieldData) {
                        $hasLocalizedValues |= isset($fieldData['data'][$lKey][$vKey]) && ($fieldData['data'][$lKey][$vKey] != NULL) && ($fieldData['isMapped'] == TRUE) && (! isset($fieldData['TCEforms']['displayCond']) || $fieldData['TCEforms']['displayCond'] != 'HIDE_L10N_SIBLINGS');
                    }
                } else {
                    if ($adminOnly == 'false') {
                        $disable = $this->MOD_SETTINGS['disablePageStructureInheritance'] != '1';
     ...
     *            Array with data from which a preview can be rendered.
     * @param array $elData
     *            Element data
     * @param array $ds_meta
     *            Data Structure Meta data
     * @param string $languageKey
     *            Current language key (so localized content can be shown)
     * @param string $sheet
     *            Sheet key
     *
                ...
                if ($fieldData['type'] == 'array') { // Making preview for array/section parts of a FlexForm structure:;
                    ...
                    if ($TCEformsConfiguration['type'] == 'group') {
                        if ($TCEformsConfiguration['internal_type'] == 'file') {
                            ...
                            $thumbnail = \TYPO3\CMS\Backend\Utility\BackendUtility::thumbCode(array(
                                'dummyFieldName' => $fieldValue
                            ), '', 'dummyFieldName', $this->doc->backPath, '', $TCEformsConfiguration['uploadfolder']);
                            ...
                            if (isset($this->renderPreviewDataObjects[$TCEformsConfiguration['allowed']]) && method_exists($this->renderPreviewDataObjects[$TCEformsConfiguration['allowed']], 'render_previewData_typeDb')) {
            ...
            if ($fieldValue['config']['type'] == 'array') {
                ...
                if (isset($fieldValue['config']['TCEforms']['config']['type']) && $fieldValue['config']['TCEforms']['config']['type'] == 'group') {
                    if ($fieldValue['config']['TCEforms']['config']['internal_type'] == 'file') {
                        // Render preview for images:
                        $thumbnail = \TYPO3\CMS\Backend\Utility\BackendUtility::thumbCode(array(
                            'dummyFieldName' => $fieldValue['data'][$vKey]
                        ), '', 'dummyFieldName', $this->doc->backPath, '', $fieldValue['config']['TCEforms']['config']['uploadfolder']);
    ...
    /**
     * Returns an HTMLized preview of a certain content element.
     * If you'd like to register a new content type, you can easily use the hook
     * provided at the beginning of the function.
     *
     * @param array $row
     *            The row of tt_content containing the content element record.
     *
     ...
     *
     * @param array $contentTreeArr
     *            Part of the contentTreeArr for the element
     * @param string $parentPointer
     *            Flexform pointer pointing to the current element (from the parent's perspective)
     * @param array $parentDsMeta
     *            Meta array from parent DS (passing information about parent containers localization mode)
     *
     * @return string HTML
                ...
                if ($sys_language_uid > 0) {
                    ...
                    switch ((string) $contentTreeArr['localizationInfo'][$sys_language_uid]['mode']) {
                        case 'exists':
                            ...
                            $l10nInfo = '<a name="c' . md5($this->apiObj->flexform_getStringFromPointer($this->currentElementParentPointer) . $localizedRecordInfo['row']['uid']) . '"></a>' . '<a name="c' . md5($this->apiObj->flexform_getStringFromPointer($this->currentElementParentPointer) . $localizedRecordInfo['row']['l18n_parent'] . $localizedRecordInfo['row']['sys_language_uid']) . '"></a>' . $this->getRecordStatHookValue('tt_content', $localizedRecordInfo['row']['uid']) . $recordIcon_l10n . htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::fixed_lgd_cs(strip_tags(\TYPO3\CMS\Backend\Utility\BackendUtility::getRecordTitle('tt_content', $localizedRecordInfo['row'])), $this->previewTitleMaxLen));
                        ...
                        case 'localize':
                            ...
                            if ((int) $contentTreeArr['el']['sys_language_uid'] === 0 && $showLocalizationLinks) {
                                ...
                                if ($this->rootElementLangParadigm == 'free') {
                                    ...
                                    $onClick = "document.location='" . $this->link_createCSRFurl([
                                        'source' => rawurlencode($sourcePointerString),
                                        'localizeElement' => $sLInfo['ISOcode']
                                    ]) . "'; return false;";
                                } else {
                                ...
                                $localizeIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-edit-copy', array(
                                    'title' => $linkLabel
                                ));
                    ...
                    if ($l10nInfo && \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->checkLanguageAccess($sys_language_uid)) {
                        $tRows[] = '
							<tr class="bgColor4">
								<td width="1%">' . $flagLink_begin . \Extension\Templavoila\Utility\IconUtility::getFlagIconForLanguage($sLInfo['flagIcon'], array(
								    'title' => $sLInfo['title'],
								    'alt' => $sLInfo['title']
								)) . $flagLink_end . '</td>
     ...
     *            DataStructure info array (the whole tree)
     * @param array $entries
     *            Entries accumulated in this array (passed by reference)
     * @param integer $indentLevel
     *            Indentation level
     * @param array $parentPointer
     *            Element position in structure
     * @param string $controls
     *            HTML for controls to add for this element
     *
     * @return void
     * @access protected
    ...
    public function render_outline_element($contentTreeArr, &$entries, $indentLevel = 0, $parentPointer = array(), $controls = '')
    {
        ...
        switch ($contentTreeArr['el']['table']) {
            case 'pages':
                ...
                $viewPageIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-view', array(
                    'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:lang/locallang_core.xlf:labels.showPage', 1)
                ));
        ...
        if ($languageUid > 0) {
            ...
            if ($this->pObj->allAvailableLanguages[$languageUid]['flagIcon']) {
                $languageIcon = \Extension\Templavoila\Utility\IconUtility::getFlagIconForLanguage($this->pObj->allAvailableLanguages[$languageUid]['flagIcon'], array(
                    'title' => $languageLabel,
                    'alt' => $languageLabel
                ));
            } else {
    ...
    public function render_outline_subElements($contentTreeArr, $sheet, &$entries, $indentLevel)
    {
        ...
        $lKeys = $langDisable ? array(
            'lDEF'
        ) : ($langChildren ? array(
            'lDEF'
        ) : $this->translatedLanguagesArr_isoCodes['all_lKeys']);
        $vKeys = $langDisable ? array(
            'vDEF'
        ) : ($langChildren ? $this->translatedLanguagesArr_isoCodes['all_vKeys'] : array(
            'vDEF'
        ));
                ...
                foreach ($contentTreeArr['sub'][$sheet][$lKey] as $fieldID => $fieldValuesContent) {
                    foreach ($vKeys as $vKey) {
                        ...
                        if (is_array($fieldValuesContent[$vKey])) {
                            ...
                            if (! $this->translatorMode) {
                                // "New" and "Paste" icon:
                                $newIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-new', array(
                ...
                if ($sys_language_uid > 0 && \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->checkLanguageAccess($sys_language_uid)) {
                    switch ((string) $contentTreeArr['localizationInfo'][$sys_language_uid]['mode']) {
                        case 'exists':
                            ...
                            // Put together the records icon including content sensitive menu link wrapped around it:
                            $recordIcon_l10n = $this->getRecordStatHookValue('tt_content', $olrow['uid']) . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconForRecord('tt_content', $olrow);
                            if (! $this->translatorMode) {
                            ...
                            $entries[] = array(
                                ...
                                'table' => 'tt_content',
                                'uid' => $olrow['uid'],
                                'flag' => $flagLink_begin . \Extension\Templavoila\Utility\IconUtility::getFlagIconForLanguage($sLInfo['flagIcon'], array(
                                    'title' => $sLInfo['title'],
                                    'alt' => $sLInfo['title']
                                )) . $flagLink_end,
     ...
     *            The label (or image)
     * @param string $table
     *            The table, fx. 'tt_content'
     * @param integer $uid
     *            The uid of the element to be edited
     * @param boolean $forced
     *            By default the link is not shown if translatorMode is set, but with this boolean it can be forced anyway.
     * @param integer $usePid
     *            ...
     * @param string $linkClass
     *            css class to use for regular content elements
     *
        ...
        if ($label) {
            ...
            if (($table == 'pages' && ($calcPerms & 2) || $table != 'pages' && ($calcPerms & 16)) && (! $this->translatorMode || $forced)) {
                if ($table == "pages" && $this->currentLanguageUid) {
                    return '<a class="tpm-pageedit btn btn-default btn-sm " href="' . $this->link_createCSRFurl([
                        'editPageLanguageOverlay' => $this->currentLanguageUid
                    ]) . '">' . $label . '</a>';
                } else {
     ...
     *            The label (or image)
     * @param array $parentPointer
     *            Flexform pointer defining the parent element of the new record
     *
     * @return string HTML anchor tag containing the label and the correct link
     * @access protected
     */
    public function link_browse($label, $parentPointer)
    {
        ...
        $onClick = 'browserPos = this;' . 'setFormValueOpenBrowser(\'db\',\'browser[communication]|||tt_content\');' . 'return false;';
    ...
    {
        $parameters = $this->link_getParameters() . '&amp;parentRecord=' . rawurlencode($this->apiObj->flexform_getStringFromPointer($parentPointer)) . '&amp;returnUrl=' . rawurlencode(\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REQUEST_URI'));
        ...
        if (strpos($label, 'icon') !== false)
            ...
            $link = '<a href="' . static::createUrlNewContentWiz($parameters) . '" title="' . \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:lang/locallang_misc.xlf:newContentElement') . '" class="btn btn-default btn-sm">' . $this->iconFactory->getIcon('actions-document-new', Icon::SIZE_SMALL)->render() . ' ' . \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL('LLL:EXT:backend/Resources/Private/Language/locallang_layout.xlf:content') . '</a>';
            return ($link);
    }
    ...
    /**
     * Returns an HTML link for unlinking a content element.
     * Unlinking means that the record still exists but
     * is not connected to any other content element or page.
     *
     * @param string $label
     *            The label
     * @param array $unlinkPointer
     *            Flexform pointer pointing to the element to be unlinked
     * @param boolean $realDelete
     *            If set, the record is not just unlinked but deleted!
     * @param boolean $foreignReferences
     *            If set, the record seems to have references on other pages
     ...
     * @param string $label
     *            The label
     * @param array $makeLocalPointer
     *            Flexform pointer pointing to the element which shall be copied
     *
     * @return string HTML anchor tag containing the label and the unlink-link
     * @access protected
     */
    ...
    {
        $output = $this->link_createCSRFurl();
            ...
            if (($commandParameters = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP($command)) != '') {
                ...
                switch ($command) {
                    ...
                    case 'editPageLanguageOverlay':
                        ...
                        if ($sys_language_uid != 0) {
                            // Edit overlay record
                            list ($pLOrecord) = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTgetRows('*', 'pages_language_overlay', 'pid=' . (int) $this->id . ' AND sys_language_uid=' . $sys_language_uid . \TYPO3\CMS\Backend\Utility\BackendUtility::deleteClause('pages_language_overlay') . \TYPO3\CMS\Backend\Utility\BackendUtility::versioningPlaceholderClause('pages_language_overlay'));
                            if ($pLOrecord) {
     ...
     *
     * @param integer $id
     *            If zero, the query will select all sys_language records from root level. If set to another value, the query will select all sys_language records that has a pages_language_overlay record on that page (and is not hidden, unless you are admin user)
     * @param boolean $onlyIsoCoded
     *            If set, only languages which are paired with a static_info_table / static_language record will be returned.
     * @param boolean $setDefault
     *            If set, an array entry for a default language is set.
     * @param boolean $setMulti
     *            If set, an array entry for "multiple languages" is added (uid -1)
     *
     * @return array
     * @access protected
     */
    public function getAvailableLanguages($id = 0, $onlyIsoCoded = TRUE, $setDefault = TRUE, $setMulti = FALSE)
    {
        ...
        if ($id) {
            ...
            $res = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTquery('DISTINCT sys_language.*, pages_language_overlay.hidden as PLO_hidden, pages_language_overlay.title as PLO_title', 'pages_language_overlay,sys_language', 'pages_language_overlay.sys_language_uid=sys_language.uid AND pages_language_overlay.pid=' . (int) $id . ' AND ' . $excludeHidden, '', 'sys_language.title');
        } else {
            $res = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTquery('sys_language.*', 'sys_language', $excludeHidden, '', 'sys_language.title');
        }
        ...
        while (TRUE == ($row = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->sql_fetch_assoc($res))) {
            ...
            if ($id) {
                ...
                $resP = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTquery('*', 'pages_language_overlay', 'pid=' . (int) $id . ' AND sys_language_uid=' . (int) $row['uid'], '', '', '1');
    ...
    /**
     * Returns label, localized and converted to current charset.
     * Label must be from FlexForm (= always in UTF-8).
     *
     * @param string $label
     *            Label
     * @param boolean $hsc
     *            <code>true</code> if HSC required
     *
     ...
     *
     * @param integer $dsUid
     *            uid of the datastructure we want to check
     * @param integer $toUid
     *            uid of the tmplobj we want to check
     *
     * @return boolean
     */
     ...
     *            the sourcePointer for the referenced element
     * @param boolean $addToSortables
     *            determine wether the element should be used for drag and drop
     *
