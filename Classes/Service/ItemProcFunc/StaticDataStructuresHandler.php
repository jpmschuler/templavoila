...
class StaticDataStructuresHandler
{
     ...
     *
     * @param
     *            array &$params Array of items passed by reference.
     * @param
     *            object &$pObj The parent object (\TYPO3\CMS\Backend\Form\FormEngine / \TYPO3\CMS\Backend\Form\DataPreprocessor depending on context)
     *            
     * @return void
     */
     ...
     *            The parent object (\TYPO3\CMS\Backend\Form\FormEngine / \TYPO3\CMS\Backend\Form\DataPreprocessor depending on context)
     *            
     * @return void
     */
    public function pi_templates(&$params, $pObj)
    {
        ...
        if ($templateRef && $storagePid) {
            ...
            $res = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTquery('title,uid,previewicon', 'tx_templavoila_tmplobj', 'tx_templavoila_tmplobj.pid=' . $storagePid . ' AND tx_templavoila_tmplobj.datastructure=' . \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->fullQuoteStr($templateRef, 'tx_templavoila_tmplobj') . ' AND tx_templavoila_tmplobj.parent=0', '', 'tx_templavoila_tmplobj.title');
            ...
            while (FALSE != ($row = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->sql_fetch_assoc($res))) {
                ...
                $params['items'][] = array(
                    \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL($row['title']),
                    $row['uid'],
                    $icon
                );
            }
    ...
    /**
     * Creates the DS selector box.
     * This function takes into account TS
     * config override of the GRSP.
     *
     * @param array $params
     *            Parameters to the itemsProcFunc
    ...
    /**
     * Retrieves DS/TO storage pid for the current page.
     * This function expectes
     * to be called from the itemsProcFunc only!
     *
     * @param array $params
     *            Parameters as come to the itemsProcFunc
     * @param \TYPO3\CMS\Backend\Form\FormEngine $pObj
     *            Calling object
     *            
