...
class Wizards
{
 ...
 // A pointer to the parent object, that is the templavoila page module script. Set by calling the method init() of this class.
    ...
    /**
     * Initializes the wizards object.
     ...
     *            Can be positive and negative depending of where the new page is going: Negative always points to a position AFTER the page having the abs. value of the positionId. Positive numbers means to create as the first subpage to another page.
     *            
     ...
     *
     * @param integer $positionPid
     *            Position id. Can be positive and negative depending of where the new page is going: Negative always points to a position AFTER the page having the abs. value of the positionId. Positive numbers means to create as the first subpage to another page.
     * @param string $templateType
     *            The template type, 'tmplobj' or 't3d'
     *            
        ...
        switch ($templateType) {
            case 'tmplobj':
                ...
                foreach ($dsList as $dsObj) {
                    ...
                    if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($disallowedPageTemplateItems, $dsObj->getKey()) || ! $dsObj->isPermittedForUser()) {
                        continue;
                    }
                    ...
                    foreach ($toList as $toObj) {
                        ...
                        if ($toObj->getKey() === $defaultTO['uid'] || ! $toObj->isPermittedForUser() || \TYPO3\CMS\Core\Utility\GeneralUtility::inList($disallowedDesignTemplateItems, $toObj->getKey())) {
                            continue;
                        }
                        ...
                        $tmplHTML[] = '<table style="width: 100%;" valign="top"><tr><td colspan="2" nowrap="nowrap"><h3 class="bgColor3-20">' . htmlspecialchars($toObj->getLabel()) . '</h3></td></tr>' . '<tr><td valign="top">' . $previewIcon . '</td><td width="120" valign="top"><p>' . \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->sL($description) . '</p></td></tr></table>';
                    }
                }
                            ...
                            if (is_array($import->dat['header']['pagetree'])) { // This means there are pages in the file, we like that...:
                                ...
                                if (is_array($import->dat['header']['thumbnail'])) {
                                    ...
                                    if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList('gif,jpg,png,jpeg', strtolower($pI['extension']))) {
                                        ...
                                        $fileName = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName('typo3temp/importthumb_' . \TYPO3\CMS\Core\Utility\GeneralUtility::shortMD5($absPath) . '.' . $pI['extension']);
                                ...
                                $aTagB = '<a href="' . htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::linkThisScript(array(
                                    'templateFile' => $absPath
                                ))) . '">';
                                $aTagE = '</a>';
     ...
     *            array containing the fields for the new page
     * @param integer $positionPid
     *            location within the page tree (parent id)
     *            
