...
class Clipboard
{
    ...
    /**
     * A pointer to the parent object, that is the templavoila page module script.
     * Set by calling the method init() of this class.
     *
     ...
     * The calling class must make sure that the right locallang files are already loaded.
     ...
     *
     * @param \tx_templavoila_module1 $pObj
     *            Reference to the parent object ($this)
     *            
     * @return void
     */
     ...
     *            Flex form pointer specifying the element we want to render the buttons for
     * @param string $listOfButtons
     *            A comma separated list of buttons which should be rendered. Possible values: 'copy', 'cut' and 'ref'
     *            
    ...
    public function element_getSelectButtons($elementPointer, $listOfButtons = 'copy,cut,ref')
    {
        ...
        $copyIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-edit-copy' . ($clipActive_copy ? '-release' : ''), array(
            'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('copyrecord')
        ));
        $cutIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-edit-cut' . ($clipActive_cut ? '-release' : ''), array(
            'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('cutrecord')
        ));
        $refIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('extensions-templavoila-clip_ref' . ($clipActive_ref ? '-release' : ''), array(
        ...
        $output = (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($listOfButtons, 'copy') && ! in_array('copy', $this->pObj->blindIcons) ? $linkCopy : '') . (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($listOfButtons, 'ref') && ! in_array('ref', $this->pObj->blindIcons) ? $linkRef : '') . (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($listOfButtons, 'cut') && ! in_array('cut', $this->pObj->blindIcons) ? $linkCut : '');
     ...
     *
     * @param array $destinationPointer
     *            Flexform pointer defining the destination location where a possible element would be pasted.
     *            
    ...
    public function element_getPasteButtons($destinationPointer)
    {
        ...
        $pasteAfterIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('extensions-templavoila-paste', array(
            'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('pasterecord')
        ));
        $pasteSubRefIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('extensions-templavoila-pasteSubRef', array(
            'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('pastefce_andreferencesubs')
        ));
    ...
    public function sidebar_renderNonUsedElements()
    {
        ...
        $res = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTquery(\TYPO3\CMS\Backend\Utility\BackendUtility::getCommonSelectFields('tt_content', '', array(
            'uid',
            'header',
            'bodytext',
            'sys_language_uid'
        )), 'tt_content', 'pid=' . (int) $pid . ' ' . 'AND uid NOT IN (' . implode(',', $usedUids) . ') ' . 'AND ( t3ver_state NOT IN (1,3) OR (t3ver_wsid > 0 AND t3ver_wsid = ' . (int) \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->workspace . ') )' . \TYPO3\CMS\Backend\Utility\BackendUtility::deleteClause('tt_content') . \TYPO3\CMS\Backend\Utility\BackendUtility::versioningPlaceholderClause('tt_content'), '', 'uid');
        ...
        while (FALSE !== ($row = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->sql_fetch_assoc($res))) {
            ...
            if ($this->pObj->allAvailableLanguages[$row['sys_language_uid']]['flagIcon']) {
                $languageIcon = \Extension\Templavoila\Utility\IconUtility::getFlagIconForLanguage($this->pObj->allAvailableLanguages[$row['sys_language_uid']]['flagIcon'], array(
                    'title' => $languageLabel,
                    'alt' => $languageLabel
                ));
            } else {
            ...
            if (! $isDeletedInWorkspace) {
                $elementRows[] = '
					<tr id="' . $elementPointerString . '" class="tpm-nonused-element">
						<td class="tpm-nonused-buttons"><div class="btn-group" role="group" aria-label="">' . $cutButton . $this->renderReferenceCount($row['uid']) . '</div></td><td class="tpm-nonused-icons">' . $recordButton . $languageIcon . '</td><td>' . htmlspecialchars(\TYPO3\CMS\Backend\Utility\BackendUtility::getRecordTitle('tt_content', $row)) . '</td>
				</tr>
			';
            }
        }
        ...
        if (count($elementRows)) {
            ...
            if (count($this->deleteUids)) {
                ...
                $deleteAll = '<a href="#" class="btn btn-default btn-sm" onclick="' . htmlspecialchars('jumpToUrl(\'' . $this->doc->issueCommand($params, - 1) . '\');') . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-edit-delete', array(
            ...
            $output = '
			    <p>' . \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('inititemno_elementsNotBeingUsed', TRUE) . ':</p>' . '<table class="tpm-nonused-elements lrPadding">
					' . implode('', $elementRows) . '
				</table>
    ...
    {
        $rows = \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->exec_SELECTgetRows('*', 'sys_refindex', 'ref_table=' . \Extension\Templavoila\Utility\GeneralUtility::getDatabaseConnection()->fullQuoteStr('tt_content', 'sys_refindex') . ' AND ref_uid=' . (int) $uid . ' AND deleted=0');
        ...
        if (count($infoData)) {
            ...
            return '<a class="tpm-countRef btn btn-default btn-sm" href="#" onclick="' . htmlspecialchars('jumpToUrl(\'' . $this->doc->issueCommand($params, - 1) . '\');') . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-edit-delete', array(
                'title' => \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('renderreferencecount_delete', TRUE)
            )) . '</a>';
        }
    }
}
