...
class Sidebar {
    ...
    /**
     * The visual position of the navigation bar.
    ...
    /**
     * Initializes the side bar object.
    ...
    public function init(&$pObj)
    {
        ...
        if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('version') && ! \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('workspaces') && \Extension\Templavoila\Utility\GeneralUtility::getBackendUser()->check('modules', 'web_txversionM1')) {
     ...
     * You are encouraged to use this function from your own extension to extend the sidebar
     * with new features. See the parameter descriptions for more details.
     *
     * @param string $itemKey
     *            A unique identifier for your sidebar item. Generally use your extension key to make sure it is unique (eg. 'tx_templavoila_sidebar_item1').
     * @param
     *            object &$object A reference to the instantiated class containing the method which renders the sidebar item (usually $this).
     * @param string $method
     *            Name of the method within your instantiated class which renders the sidebar item. Case sensitive!
     * @param string $label
     *            The label which will be shown for your item in the sidebar menu. Make sure that this label is localized!
     * @param integer $priority
     *            An integer between 0 and 100. The higher the value, the higher the item will be displayed in the sidebar. Default is 50
     ...
     *
     * @param string $itemKey
     *            The key identifying the sidebar item.
     *            
     * @return void
     */
    ...
    public function render()
    {
        if (is_array($this->sideBarItems) && count($this->sideBarItems)) {
            uasort($this->sideBarItems, array(
                $this,
                'sortItemsCompare'
            ));
    ...
    /**
     * Renders the versioning sidebar item.
     * Basically this is a copy from the template class.
     *
     * @param
     *            \tx_templavoila_module1 &$pObj Reference to the page object (the templavoila page module)
     *            
     * @return string HTML output
     */
